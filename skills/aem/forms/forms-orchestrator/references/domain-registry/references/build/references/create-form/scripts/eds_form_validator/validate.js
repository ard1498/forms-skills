 ...
 *
 * Usage:
 *   node validate.js <form.json>
 *   node validate.js <form.json> <_form.json>
 *   node validate.js <form.json> --json
 */

import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import { validate } from './FormFieldValidator.js';
...
Usage:
  node validate.js <form.json>                        Validate a form file
  node validate.js <form.json> <_form.json>           Validate with custom components from _form.json
  node validate.js <form.json> --json                 Output JSON report only (for LLM)
  node validate.js <form.json> <_form.json> --json    Both options combined
  node validate.js --help                             Show this help
...
What it validates:
  ...
  - Fragments: Auto-detects panel fields that are fragments (via sling:resourceType,
    aueComponentId, or fragmentPath) and validates using form-fragment schema

Examples:
  node validate.js ./form.json
  node validate.js ./form.json ./authoring/_form.json
  node validate.js /path/to/my-form.json --json
`);
