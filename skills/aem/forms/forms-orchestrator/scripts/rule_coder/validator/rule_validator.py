...
class RuleValidator:
    ...
    def _validate_statement(self, node: Dict[str, Any]) -> List[str]:
        ...
        if choice.get("nodeName") != "TRIGGER_SCRIPTS":
            errors.append(
                "STATEMENT choice must be TRIGGER_SCRIPTS for trigger event rules"
            )
            return errors
        ...
        if (
            "items" in choice
            and isinstance(choice["items"], list)
            and len(choice["items"]) > 0
        ):
            trigger_scripts = choice["items"][0]
            if (
                isinstance(trigger_scripts, dict)
                and trigger_scripts.get("nodeName") == "SINGLE_TRIGGER_SCRIPTS"
            ):
    ...
    def _validate_single_trigger_scripts(self, node: Dict[str, Any]) -> List[str]:
        ...
        if len(items) < 4:
            errors.append(
                f"SINGLE_TRIGGER_SCRIPTS must have at least 4 items, found {len(items)}"
            )
            return errors
        ...
        # Check TRIGGER_EVENT
        if (
            not isinstance(items[1], dict)
            or items[1].get("nodeName") != "TRIGGER_EVENT"
        ):
        ...
        if not isinstance(items[2], dict) or items[2].get("nodeName") != "When":
            errors.append(
                "Third item in SINGLE_TRIGGER_SCRIPTS must be 'When' literal token"
            )

        # Check TRIGGER_EVENT_SCRIPTS
        if (
            not isinstance(items[3], dict)
            or items[3].get("nodeName") != "TRIGGER_EVENT_SCRIPTS"
        ):
            errors.append(
                "Fourth item in SINGLE_TRIGGER_SCRIPTS must be TRIGGER_EVENT_SCRIPTS"
            )
        else:
    ...
    def _validate_trigger_event_scripts(self, node: Dict[str, Any]) -> List[str]:
        ...
        if len(items) < 3:
            errors.append(
                f"TRIGGER_EVENT_SCRIPTS must have at least 3 items, found {len(items)}"
            )
            return errors
        ...
        if not isinstance(items[0], dict) or items[0].get("nodeName") != "CONDITION":
            ...
            if items[0].get("nested") is None:
                errors.append(
                    "CONDITION node must have 'nested' property (should be false)"
                )

        # Check "Then" token
        if not isinstance(items[1], dict) or items[1].get("nodeName") != "Then":
            errors.append(
                "Second item in TRIGGER_EVENT_SCRIPTS must be 'Then' literal token"
            )

        # Check BLOCK_STATEMENTS (then actions)
        if (
            not isinstance(items[2], dict)
            or items[2].get("nodeName") != "BLOCK_STATEMENTS"
        ):
            errors.append(
                "Third item in TRIGGER_EVENT_SCRIPTS must be BLOCK_STATEMENTS"
            )
        else:
        ...
        if len(items) > 3:
            ...
            if not isinstance(items[3], dict) or items[3].get("nodeName") != "Else":
                errors.append(
                    "Fourth item in TRIGGER_EVENT_SCRIPTS must be 'Else' literal token"
                )

            # Check BLOCK_STATEMENTS (else actions)
            if len(items) > 4:
                if (
                    not isinstance(items[4], dict)
                    or items[4].get("nodeName") != "BLOCK_STATEMENTS"
                ):
                    errors.append(
                        "Fifth item in TRIGGER_EVENT_SCRIPTS must be BLOCK_STATEMENTS"
                    )
                else:
    ...
    def _validate_block_statements(self, node: Dict[str, Any]) -> List[str]:
        ...
        for i, item in enumerate(node["items"]):
            ...
            if item.get("nodeName") != "BLOCK_STATEMENT":
                errors.append(
                    f"BLOCK_STATEMENTS item {i} must have nodeName='BLOCK_STATEMENT'"
                )
                continue
            ...
            if action_name not in self._supported_actions:
                errors.append(
                    f"Action '{action_name}' is not supported in simplified grammar"
                )
        ...
        if isinstance(node, dict):
            ...
            if node_name:
                ...
                if node_name in NOT_SUPPORTED_NODES:
                    errors.append(
                        f"Node '{node_name}' is not supported in simplified grammar. Use FUNCTION_CALL instead."
                    )

                # Check if node is a known node (or literal token)
                if (
                    node_name not in self._supported_nodes
                    and node_name not in LITERAL_TOKENS
                ):
                    # Reject unknown node names - NEVER allow invented nodeNames
                    if not self._is_known_pattern(node_name):
                        errors.append(
                            f"Unknown node name '{node_name}' - not in grammar whitelist. Only use nodeNames from annotated_subset_grammar.json"
                        )
    ...
    def _is_known_pattern(self, node_name: str) -> bool:
        ...
        return (
            node_name in operators
            or node_name in nav_options
            or node_name in bool_rules
        )
            ...
            # Check function references
                ...
                if isinstance(func_name, dict):
                    ...
                    if func_id and self.form_context:
                        ...
                        if function is None:
                            warnings.append(
                                f"Function not found in context: {func_id} (may be custom function)"
                            )
...
def validate_rule(
    rule: Dict[str, Any], form_context: Optional[Any] = None
) -> ValidationResult:
    """
    Validate a rule JSON.
