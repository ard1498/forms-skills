...
// scripts/src/validate-custom-function.js
var validate_custom_function_exports = {};
__export(validate_custom_function_exports, {
  validateCustomFunctionFile: () => validateCustomFunctionFile,
  validateCustomFunctionSource: () => validateCustomFunctionSource
});
module.exports = __toCommonJS(validate_custom_function_exports);
...
// scripts/src/validate-custom-function.js
var __dirname = import_node_path.default.dirname((0, import_node_url.fileURLToPath)(__import_meta_url__));
function loadJson(relPath) {
  const fullPath = import_node_path.default.join(__dirname, relPath);
  return JSON.parse(import_node_fs.default.readFileSync(fullPath, "utf8"));
}
var SCOPE_FUNCTIONS = loadJson("./definitions/scope-functions.json");
var TYPES = loadJson("./definitions/types.json");
...
function extractQualifiedNamesFromTree(treeJson) {
  ...
  function visit(node) {
    if (node.id && node.fieldType) {
      result[node.id] = { type: node.fieldType, customProperties: node.customProperties ?? [] };
    }
    if (node.items) node.items.forEach(visit);
  }
  if (treeJson) visit(treeJson);
  return result;
}
...
function getScopeParamFromJSDoc(jsdoc) {
  ...
  const parsed = parse4(jsdoc)[0];
  const scopeTag = parsed?.tags?.find(
    (t) => t.tag === "param" && t.type === "scope"
  );
...
function validateScopeParam(fnNode, scopeName, errors) {
  ...
  if (!last || last.type !== "Identifier" || last.name !== scopeName) {
    errors.push(
      diag("SCOPE_PARAM_LAST", "scope parameter must be last", fnNode)
    );
  }
}
...
var VALID_ROOTS = /* @__PURE__ */ new Set(["form", "fragment"]);
var SKIP_ROOTS = /* @__PURE__ */ new Set(["functions", "field"]);
...
function validateCall(node, scopeFunctions, qualifiedNames, customValidators, errors, aliasMap) {
  ...
  if (args.length < minArgs || args.length > maxArgs) {
    errors.push(
      diag(
        "ARG_COUNT_INVALID",
        `invalid arg count for ${fnName}: expected ${minArgs}-${maxArgs}, got ${args.length}`,
        node
      )
    );
    return;
  ...
  def.args.forEach((spec, i) => {
    ...
    if (!arg && spec.required) {
      errors.push(
        diag(
          "ARG_MISSING",
          `missing required arg '${spec.name}' for ${fnName}`,
          node
        )
      );
      return;
    ...
    if (!spec.kind.includes(kind) && !spec.kind.includes("any")) {
      ...
      const code = spec.kind.includes("qualifiedName") && kind === "object" || spec.kind.includes("object") && kind === "qualifiedName" ? "ARG_ORDER_INVALID" : "ARG_TYPE_INVALID";
      errors.push(
        diag(
          code,
          `arg '${spec.name}' must be ${spec.kind.join("|")}, got ${kind}`,
          arg
        )
      );
    }
  ...
  if (customValidators?.[fnName]) {
    const mappedArgs = args.map((arg, i) => ({
      ...
      value: arg.type === "ObjectExpression" ? Object.fromEntries(
        arg.properties.filter((p) => p.key && p.value).map((p) => [p.key.name, p.value.value])
      ) : void 0
    }));
    const customErrors = customValidators[fnName](mappedArgs, {
      qualifiedNames,
      types: TYPES
    });
...
function validateCustomFunctionSource(source, options = {}) {
  ...
  try {
    ...
    return {
      valid: false,
      errors: [diag("SYNTAX_ERROR", e.message, e.loc ? { loc: e.loc } : null)],
      warnings,
      metadata: {}
    };
  }
  ...
  function validateMemberExpr(node, scopeName, aliasMap) {
    ...
    if (!resolved) {
      errors.push(
        diag(
          "QUALIFIED_NAME_UNKNOWN",
          `unknown qualified name: ${parts.slice(1).join(".")}`,
          node
        )
      );
      return;
    ...
    if (resolved.invalidRoot) {
      errors.push(
        diag(
          "QUALIFIED_NAME_INVALID_ROOT",
          `invalid root '${resolved.invalidRoot}': must be 'form' or 'fragment'`,
          node
        )
      );
      return;
    ...
    if (!meta?.type) {
      errors.push(
        diag(
          "QUALIFIED_NAME_MISSING_TYPE",
          `qualified name missing type: ${resolved.key}`,
          node
        )
      );
      return;
      ...
      if (!isCustomPropertyPath(resolved.rest, meta.customProperties || [])) {
        errors.push(
          diag(
            ...
            `invalid custom property path: ${resolved.rest.join(".")}`,
            node
          )
        );
      }
    ...
    const typeSpecificExposed = new Set(
      TYPES.runtime.typeSpecificExposedProperties?.[typeKey] || []
    );
    const typeSpecificModifiable = new Set(
      TYPES.runtime.ruleModifiableByType?.[typeKey] || []
    );
    const allowed = /* @__PURE__ */ new Set([
      ...commonProps,
      ...modifiableCommon,
      ...typeSpecificExposed,
      ...typeSpecificModifiable
    ]);
    if (!allowed.has(prop)) {
      ...
      if (isContainer && !prop.startsWith("$")) {
        ...
        if (!options.qualifiedNames[childKey]) {
          errors.push(
            diag(
              "QUALIFIED_NAME_UNKNOWN",
              `unknown qualified name: ${resolved.key.replace(/^\$/, "")}.${prop}`,
              node
            )
          );
          return;
        ...
        errors.push(
          diag(
            "PROPERTY_INVALID",
            `property '${prop}' not valid for type '${meta.type}'`,
            node
          )
        );
      }
  ...
  for (const name of exported) {
    ...
    if (!fn) {
      errors.push(
        diag("EXPORT_NO_FUNCTION", `exported function ${name} not found`, ast)
      );
      continue;
    }
  ...
  for (const name of exported) {
    ...
    simple(fn, {
      MemberExpression(node) {
        validateMemberExpr(node, effectiveScopeName, aliasMap);
      },
  ...
  return {
    ...
    errors,
    warnings,
    metadata: { scopeFunctions: SCOPE_FUNCTIONS, types: TYPES }
  };
}
async function validateCustomFunctionFile(filePath, options = {}) {
  const src = import_node_fs.default.readFileSync(filePath, "utf8");
...
if (process.argv[1] === (0, import_node_url.fileURLToPath)(__import_meta_url__)) {
  try {
    ...
    if (!sourcePath) {
      process.stdout.write(
        JSON.stringify({
          ...
          errors: [{
            code: "CLI_ERROR",
            message: "Usage: validate-custom-function.bundle.js <functions.js> [--qualified-names <qn.json>] [--tree <treeJson.json>]"
          }],
          warnings: []
        }) + "\n"
      );
    ...
    if (qnIdx !== -1 && !cliArgs[qnIdx + 1]) {
      process.stdout.write(JSON.stringify({ valid: false, errors: [{ code: "CLI_ERROR", message: "--qualified-names requires a file path argument" }], warnings: [] }) + "\n");
      process.exit(2);
    }
    if (qnIdx !== -1) {
      qualifiedNames = JSON.parse(
        import_node_fs.default.readFileSync(cliArgs[qnIdx + 1], "utf8")
      );
    } else {
      const treeIdx = cliArgs.indexOf("--tree");
      if (treeIdx !== -1 && !cliArgs[treeIdx + 1]) {
        process.stdout.write(JSON.stringify({ valid: false, errors: [{ code: "CLI_ERROR", message: "--tree requires a file path argument" }], warnings: [] }) + "\n");
        process.exit(2);
      }
      if (treeIdx !== -1) {
        const treeJson = JSON.parse(
          import_node_fs.default.readFileSync(cliArgs[treeIdx + 1], "utf8")
        );
        qualifiedNames = extractQualifiedNamesFromTree(treeJson);
      }
    }
    const source = import_node_fs.default.readFileSync(sourcePath, "utf8");
...
0 && (module.exports = {
  validateCustomFunctionFile,
  validateCustomFunctionSource
});
