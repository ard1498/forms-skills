...
(async () => {
  ...
  if (rawArgs.includes("--help") || rawArgs.includes("-h")) {
    process.stdout.write(`${[
      ...
      "",
      "Output (success): { success: true, input, fdEvents, fdRules, formulaValid: true }",
      "Output (failure): { success: false, error, formulaValid: false }",
...
`);
  ...
  const fdRulesNode = { [fdKey]: [JSON.stringify(ruleJson)] };
  ...
  const check = (loc, script) => {
    ...
    const r = RuleTransformer.validateFormula(script);
    if (!r.valid) {
...
`);
  ...
  process.stdout.write(`${JSON.stringify({ success: false, error: e.message, formulaValid: false })}
`);
  process.exit(1);
});
