...
function validateCall(
  ...
  def.args.forEach((spec, i) => {
    ...
    if (!spec.kind.includes(kind) && !spec.kind.includes("any")) {
      ...
      const code =
        (spec.kind.includes("qualifiedName") && kind === "object") ||
...
export function validateCustomFunctionSource(source, options = {}) {
  ...
  function validateMemberExpr(node, scopeName, aliasMap) {
    ...
    if (
      !TYPES.supportedFieldTypes.includes(meta.type) &&
      meta.type !== "form" &&
      meta.type !== "panel"
    ) {
  ...
  for (const name of exported) {
    ...
    const effectiveScopeName =
      scopeName ?? (lastParam?.type === "Identifier" ? lastParam.name : null);
...
// Run as CLI when invoked directly; skipped when bundled output is required() by other code.
